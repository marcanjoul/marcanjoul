...
\
`Python` • `scikit-learn` • `pandas` • `GridSearchCV` • `Jupyter Notebook`

### 📝 [NoteWeb](https://github.com/marcanjoul/NoteWeb)
Local-first RAG system — semantic search across PDFs with embeddings, vector DB, and LLaMA 3.
\
`Python` • `Ollama (LLaMA 3)` • `Embeddings` • `Chunking + Caching` • `Conversational Q&A`

### 🎶 [Groov](https://github.com/marcanjoul/Groov)
Album explorer using Last.fm API with star ratings and to-listen lists.
\
`Swift` • `UIKit` • `URLSession` • `Async API Calls` • `Custom UICollectionView`
